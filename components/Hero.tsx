...
const Hero: React.FC<HeroProps> = ({ t }) => {
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-amber-50 via-white to-amber-100 pt-20">
      ...
      <div className="relative z-10 px-6 w-full max-w-7xl grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center text-white">
        <div className="space-y-6 max-w-2xl">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="Pharma Ruche logo featuring Arabic text Osama Al-Sagheer Apiaries, representing a honey production company with warm golden tones" className="w-40 drop-shadow-lg" />
          </div>
          <div className="inline-block px-4 py-2 rounded-full border border-amber-200/40 bg-white/10 backdrop-blur-sm text-amber-100 font-bold text-xs uppercase tracking-[0.3em] animate-fade-in-down">
            ✨ {t('tagline')}
          </div>

          <h1 className="text-5xl md:text-6xl lg:text-7xl font-black leading-[1.1] drop-shadow-lg animate-fade-in font-cairo">
            {t('hero_title')}
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 animate-fade-in-up pt-4">
            <a
              href="#products"
              className="w-full sm:w-auto bg-[#d97706] hover:bg-[#b45309] text-white font-bold py-4 px-12 rounded-lg transition-all transform hover:scale-105 shadow-xl shadow-amber-900/30 flex items-center justify-center gap-2 text-sm uppercase tracking-widest"
            >
              🛍️ {t('products')}
            </a>
            <a
              href="#about"
              className="w-full sm:w-auto bg-white/10 hover:bg-white/20 text-white font-bold py-4 px-12 rounded-lg transition-all flex items-center justify-center text-sm uppercase tracking-widest border border-white/30 backdrop-blur-sm"
            >
              📖 {t('about_us')}
            </a>
          </div>
