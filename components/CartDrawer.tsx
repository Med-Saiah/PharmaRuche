...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose, cart, updateQuantity, removeFromCart, onPlaceOrder, t, lang }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000]">
      ...
      <div className={`absolute top-0 ${lang === 'ar' ? 'left-0' : 'right-0'} w-full max-w-lg h-full bg-white shadow-2xl flex flex-col animate-in ${lang === 'ar' ? 'slide-in-from-left' : 'slide-in-from-right'} duration-500`}>
          ...
          {cart.length === 0 ? (
            ...
            <>
              <div className="space-y-4">
                {cart.map(item => (
                  <div key={item.id} className="group flex gap-5 p-5 bg-zinc-50 rounded-[2rem] border border-zinc-100 transition-all hover:bg-white hover:shadow-xl hover:shadow-zinc-200/50">
                    <img src={item.image} className="w-20 h-20 rounded-2xl object-cover shadow-sm" alt="" />
                    <div className="flex-1 flex flex-col justify-between">
                      <div className="flex justify-between items-start">
                        <h4 className="font-black text-zinc-900 leading-tight pr-4">{item.name[lang]}</h4>
                        <button 
                      ...
                      <div className="flex justify-between items-center mt-3">
                        ...
                        <span className="font-black text-[#d97706]">{item.price * item.quantity} {t('currency')}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="pt-10 space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input className="w-full p-5 bg-zinc-50 rounded-2xl border border-zinc-200 outline-none focus:ring-4 focus:ring-zinc-900/5 focus:border-zinc-950 transition-all font-bold" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-zinc-400 ml-4">{t('phone')}</label>
                    <input type="tel" className="w-full p-5 bg-zinc-50 rounded-2xl border border-zinc-200 outline-none focus:ring-4 focus:ring-zinc-900/5 focus:border-zinc-950 transition-all font-bold" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-zinc-400 ml-4">{t('wilaya')}</label>
                    <select className="w-full p-5 bg-zinc-50 rounded-2xl border border-zinc-200 outline-none font-bold appearance-none cursor-pointer" value={formData.wilaya} onChange={(e) => setFormData({ ...formData, wilaya: e.target.value })}>
                      {WILAYAS.map(w => <option key={w} value={w}>{w}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-zinc-400 ml-4">{t('address')}</label>
                    <input className="w-full p-5 bg-zinc-50 rounded-2xl border border-zinc-200 outline-none focus:ring-4 focus:ring-zinc-900/5 focus:border-zinc-950 transition-all font-bold" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} />
                  </div>
                </div>
              </div>
            </>
        ...
        {cart.length > 0 && (
          <div className="p-8 bg-white border-t border-zinc-100 space-y-6 shadow-[0_-20px_50px_-20px_rgba(0,0,0,0.1)]">
